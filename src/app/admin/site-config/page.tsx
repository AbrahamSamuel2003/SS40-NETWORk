...
export default function SiteConfigPage() {
    ...
    const [isUploadingContactImage, setIsUploadingContactImage] = React.useState(false);
                ...
                if (res.ok && data.success) {
                    if (isMounted) {
                        setConfig({
                            ...
                            contactSectionImageUrl: data.data.contactSectionImageUrl || '',
    ...
    const handleContactImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const res = await fetch('/api/admin/site-config/upload-contact-image', {
                method: 'POST',
                body: formData,
            });
            ...
            const data = await res.json();
            if (res.ok && data.success) {
                setConfig(prev => prev ? { ...prev, contactSectionImageUrl: data.url } : prev);
                setSuccessMsg('Contact image uploaded successfully. Click Save Changes to apply it.');
            } else {
                setErrorMsg(data.error || 'Failed to upload contact image.');
            }
        } catch {
            setErrorMsg('An error occurred during contact image upload.');
        } finally {
    ...
    const handleRemoveContactImage = () => {
        setConfig(prev => prev ? { ...prev, contactSectionImageUrl: '' } : prev);
    };
            ...
            <form onSubmit={handleSave} className="space-y-8">
                ...
                <div className="bg-[#111111] border border-white/10 rounded-xl overflow-hidden p-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-white/80 mb-2">Contact Section Image</label>
                            {config.contactSectionImageUrl ? (
                                <div className="flex items-center gap-4 bg-black/30 border border-white/10 rounded-lg p-4">
                                    {/* eslint-disable-next-line @next/next/no-img-element */}
                                    <img src={config.contactSectionImageUrl} alt="Contact section preview" className="h-24 w-32 object-cover bg-white/10 rounded-lg" />
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm text-white/70 mb-1">Image shown in the Visit Us contact card.</p>
                                        <p className="text-xs text-white/40 truncate">{config.contactSectionImageUrl}</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={handleRemoveContactImage}
                                        className="text-red-400 hover:text-red-300 p-2 border border-red-900/30 rounded-lg hover:bg-red-500/10 transition-colors"
                                        title="Remove contact image"
                                    >
                                        <Trash2 className="w-5 h-5" />
                                    </button>
                                </div>
                                ...
                                <div className="border border-dashed border-white/20 rounded-lg p-6 bg-black/30 flex flex-col items-center justify-center text-center">
                                    ...
                                    />
                                    <label
                                        htmlFor="contact-image-upload-input"
                                        className={`cursor-pointer inline-flex items-center gap-2 px-4 py-2 ${isUploadingContactImage ? 'bg-white/10 text-white/50' : 'bg-[var(--color-primary)] text-white hover:bg-blue-600'} rounded-lg transition-colors font-medium text-sm`}
                                    >
                                    ...
                                    <p className="text-xs text-white/40 mt-3">Upload the image used in the Contact page Visit Us card.</p>
                                </div>
                            )}
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-white/80 mb-2">Contact Section Image URL</label>
                            <input
                                type="text"
                                name="contactSectionImageUrl"
                                value={config.contactSectionImageUrl || ''}
                                onChange={handleChange}
                                className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] transition-all"
                                placeholder="https://example.com/contact-office.jpg"
                            />
                            <p className="mt-1 text-xs text-white/40">Optional. Uploaded images fill this automatically.</p>
                        </div>
                        <div>
