    ...
    return (
        <div className="max-w-6xl mx-auto pb-12">
            <div className="mb-8 flex justify-between items-end">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-[#111827] mb-2">Home Page Happimonials</h2>
                    <p className="text-[#6B7280]">Manage content for the Home page "Success Stories" section.</p>
                </div>
                ...
                <table className="w-full text-left text-sm text-[#374151]">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {happimonials.length === 0 ? (
                            <tr><td colSpan={4} className="p-8 text-center text-[#9CA3AF]">No entries specific to Home found.</td></tr>
                        ) : (
                            happimonials.map(item => (
                                <tr key={item.id} className="hover:bg-[#EDF5F2]/50">
                                    ...
                                    <td className="p-4 text-right">
                                        <button onClick={() => handleOpenModal(item)} className="text-[#9CA3AF] hover:text-[#111827] p-2">
                                            <Edit2 className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(item.id)} className="text-[#B91C1C]/50 hover:text-[#B91C1C] p-2">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#111827]/40 backdrop-blur-sm">
                    <div className="admin-card w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 overflow-y-auto w-full">
                            ...
                            <form id="happimonialForm" onSubmit={handleSave} className="space-y-5">
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm text-[#374151] mb-1.5">Client Name *</label><input required value={clientName} onChange={e => setClientName(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[#111827]" /></div>
                                    <div><label className="block text-sm text-[#374151] mb-1.5">Company Name *</label><input required value={companyName} onChange={e => setCompanyName(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[#111827]" /></div>
                                </div>
                                <div><label className="block text-sm text-[#374151] mb-1.5">Industry Category *</label><input required value={industry} onChange={e => setIndustry(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[#111827]" /></div>

                                <div><label className="block text-sm text-[#374151] mb-1.5">Testimonial Quote *</label><textarea required rows={4} value={testimonial} onChange={e => setTestimonial(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[#111827] resize-none" /></div>

                                <div>
                                    <label className="block text-sm text-[#374151] mb-1.5">Avatar Image</label>
                                ...
                                <div>
                                    <label className="block text-sm text-[#374151] mb-1.5">
                                        YouTube Video URL
                                        <span className="ml-2 text-[#9CA3AF] text-xs font-normal">(optional — only one allowed per Home section)</span>
                                    </label>
                                    <input
                                        value={youtubeUrl}
                                        onChange={e => setYoutubeUrl(e.target.value)}
                                        className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[#111827] placeholder:text-[#9CA3AF]"
                        ...
                        <div className="p-5 border-t border-gray-200 flex justify-end gap-3 bg-[#EDF5F2]/70">
                            ...
                            <button type="submit" form="happimonialForm" disabled={isSaving} className="bg-[#6B9F91] hover:bg-[#5C8C80] text-[#111827] px-6 py-2 rounded-lg disabled:opacity-50">Save Happimonial</button>
                        </div>
                    </div>
                </div>
            )}
