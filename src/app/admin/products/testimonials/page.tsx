    ...
    return (
        <div className="max-w-6xl mx-auto pb-12">
            <div className="mb-8 flex justify-between items-end">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white mb-2">Product Testimonials</h2>
                    <p className="text-white/60">Manage client success stories for the Products section.</p>
                </div>
                ...
                <table className="w-full text-left text-sm text-white/80">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {happimonials.length === 0 ? (
                            <tr><td colSpan={4} className="p-8 text-center text-white/40">No product testimonials found.</td></tr>
                        ) : (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="bg-[#111111] border border-white/10 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 overflow-y-auto w-full custom-scrollbar">
                            ...
                            <form id="happimonialForm" onSubmit={handleSave} className="space-y-5">
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm text-white/80 mb-1.5">Client Name *</label><input required value={clientName} onChange={e => setClientName(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white" /></div>
                                    <div><label className="block text-sm text-white/80 mb-1.5">Company Name *</label><input required value={companyName} onChange={e => setCompanyName(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white" /></div>
                                </div>
                                <div><label className="block text-sm text-white/80 mb-1.5">Industry Category *</label><input required value={industry} onChange={e => setIndustry(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white" /></div>

                                <div><label className="block text-sm text-white/80 mb-1.5">Testimonial Quote *</label><textarea required rows={4} value={testimonial} onChange={e => setTestimonial(e.target.value)} className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white resize-none" /></div>

                                <div>
                                    <label className="block text-sm text-white/80 mb-1.5">Avatar / Thumbnail Image</label>
                                ...
                                <div>
                                    <label className="block text-sm text-white/80 mb-1.5">
                                        YouTube Video URL
                                        <span className="ml-2 text-white/30 text-xs font-normal">(optional)</span>
                                    </label>
                                    <input
                                        value={youtubeUrl}
                                        onChange={e => setYoutubeUrl(e.target.value)}
                                        className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-white placeholder-white/20"
                        ...
                        <div className="p-5 border-t border-white/10 flex justify-end gap-3 bg-black/20">
                            ...
                            <button type="submit" form="happimonialForm" disabled={isSaving} className="bg-[var(--color-primary)] hover:bg-blue-600 text-white px-6 py-2 rounded-lg disabled:opacity-50">Save Testimonial</button>
                        </div>
                    </div>
                </div>
            )}
